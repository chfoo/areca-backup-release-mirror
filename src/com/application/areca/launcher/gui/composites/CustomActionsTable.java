 ...
 */
public class CustomActionsTable {
    ...
    public CustomActionsTable(Composite parent, TargetEditionWindow tge, boolean preprocess) {
        ...
        btnAddProc.addListener(SWT.Selection, new Listener(){
            public void handleEvent(Event event) {
                CustomAction newproc = showProcEditionFrame(null);
                if (newproc != null) {
    ...
    private void editCurrentProcessor() {
        if (tblProc.getSelectionIndex() != -1) {
            ...
            CustomAction proc = (CustomAction)item.getData();
    ...
    private CustomAction showProcEditionFrame(CustomAction proc) {
        ProcessorEditionWindow frm = new ProcessorEditionWindow(proc, (FileSystemRecoveryTarget)main.getTarget(), preprocess);
        main.showDialog(frm);
        CustomAction prc = frm.getCurrentProcessor();
        return prc;
    }


    private void addProcessor(CustomAction proc) {
        TableItem item = new TableItem(tblProc, SWT.NONE);
        updateProcessor(item, proc);
    }

    private void updateProcessor(TableItem item, CustomAction proc) {
    ...
    public void addProcessors(CustomActionList list) {
        for (int i=0; i<tblProc.getItemCount(); i++) {
            list.addAction((CustomAction)tblProc.getItem(i).getData());
        }
    }

    public void setProcessors(CustomActionList list) {
        Iterator processors = list.iterator();
        int index = tblProc.getSelectionIndex();
        while (processors.hasNext()) {
            CustomAction proc = (CustomAction)processors.next();
